...
| T62 | Replace N.O.R.A. mockup with real `doc/img/noraapp.png` | T62 | ✅ DONE | User: "homepage. i want to replace this with the image from `/doc/imag/noraapp.png`" (typo — actual path `doc/img/noraapp.png`). Replaced the 33-line hand-coded phone mockup (探索宇宙 / 學習進度 28% / 火箭工程 / 軌道與衛星 / 航天英雄 / 繼續學習 cards) with a single `<img src="doc/img/noraapp.png">` (853×1844 PNG, rendered at `w-[260px]` to match the previous mockup width). `rounded-[28px]` + `border` + `shadow-[var(--elev-2)]` preserved the phone-frame aesthetic. | `index.html`, `.kilo/plans/REVAMP_PLAN_V3.md` |
| T63 | Drop 「APP 上線時通知我」 email-notify card on homepage | T63 | ✅ DONE | User: "remove this part" (the email-notify card with 電郵地址 input + 登記通知 button on the homepage N.O.R.A. section). Removed the entire `<form>` block (label + email input + submit button + privacy note). The §07 ECOSYSTEM column now ends after the 4-bullet value-prop list. No JS or layout change beyond the removed block. | `index.html`, `.kilo/plans/REVAMP_PLAN_V3.md` |
...
| 2026-09-06 | User → Kilo | **T62** (homepage N.O.R.A. mockup → real image) | Replaced the 33-line hand-coded N.O.R.A. Kids phone mockup in §07 ECOSYSTEM of `index.html` with a single `<img src="doc/img/noraapp.png" alt="...">` (853×1844 PNG, w-[260px] frame + rounded-[28px] + border + shadow). User typo'd the path (`/doc/imag/`) — actual file is `doc/img/noraapp.png`. | `index.html`, `.kilo/plans/REVAMP_PLAN_V3.md` |
| 2026-09-06 | User → Kilo | **T63** (drop homepage email-notify) | Removed the entire 「APP 上線時通知我」 email-notify card from §07 ECOSYSTEM of `index.html` (label + email input + 登記通知 button + privacy note + form wrapper). Section column now ends after the 4-bullet value-prop list. | `index.html`, `.kilo/plans/REVAMP_PLAN_V3.md` |
