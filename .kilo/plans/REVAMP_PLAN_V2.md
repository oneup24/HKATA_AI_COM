...
| 2026-09-06 | User → Kilo | **T26** (Gap fix — actually applied) | Breadcrumb `<nav>` was sitting OUTSIDE the hero `<section>` with its own `pt-40`, AND the hero had its own `pt-24`. Stacked padding = ~256px blank gap. Moved the breadcrumb INSIDE the hero as the first child; removed its `pt-40` and its redundant `container-x` wrapper (the hero's own `container-x` covers it). The hero's `pt-24` is now the sole element clearing the fixed header. | `kindergarten.html`, `primary.html`, `secondary.html` |
